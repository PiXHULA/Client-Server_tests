...
public class Database {
    ...
    public Person getPersonBySomething(String s){
        for(Person p:AddressBook){
            if(p.getAddress().equalsIgnoreCase(s)
                    ||p.getName().equalsIgnoreCase(s)
            ||p.getBirthday().equalsIgnoreCase(s)
            ||p.getPhoneNumber().equalsIgnoreCase(s))
                return p;
        }
        return null;
    }
